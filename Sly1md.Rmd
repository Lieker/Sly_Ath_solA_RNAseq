...
Check if anything improves when we use adjusted filtering with IHW.

```{r IHW, message = FALSE, warning = FALSE}
resrIHW <- results(dds, filterFun = ihw)
summary(resrIHW)
sum(resrIHW$padj < 0.1, na.rm=TRUE)
metadata(resrIHW)$ihwResult
sum(resrIHW$padj < 0.05, na.rm=TRUE)
```

```{r check_root_volcano, message = FALSE, warning = FALSE}
ddss <- get_DESeq_dds(plantpart = "root", method = "treatment")
ress <- get_unfiltered_res_dds(plantpart = "root", method = "treatment")
ressd <- as.data.frame(ress)
ggplot(data=ressd, aes(x=log2FoldChange, y=-log10(padj))) + geom_point() + xlim(c(-10,10)) + theme_classic() +
    geom_hline(yintercept=-log10(0.05), col="red")
```

Check this with a scatterplot of pvalues

```{r scatterplot_pvalues, message = FALSE, warning = FALSE}
plot(resr$baseMean+1, -log10(resr$pvalue),
     log="x", xlab="mean of normalized counts",
     ylab=expression(-log[10](pvalue)),
     ylim=c(0,30),
     cex=.4, col=rgb(0,0,0,.3))

plot(resrIHW$baseMean+1, -log10(resrIHW$pvalue),
     log="x", xlab="mean of normalized counts",
     ylab=expression(-log[10](pvalue)),
     ylim=c(0,30),
     cex=.4, col=rgb(0,0,0,.3))
...
There are no differences at all between resrIHW and resr.
