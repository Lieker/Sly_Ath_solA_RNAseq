...
```{r}
p <- plot_pca(plantpart = "root", pc_x_axis = "PC5", pc_y_axis = "PC1", pca_shape = "treatment")
p + scale_color_manual(values=c('#e956b4', '#56e98b'), name = "solanoeclepin A", labels = c("no", "yes")) + guides(shape = FALSE)
```
...
p + scale_color_manual(values=c('#566be9', '#d71c37'), name = "solanoeclepin A", labels = c("no", "yes")) + guides(shape = FALSE)
```
...
clrs <- list(
  solA = c(no = "gray", yes ="midnightblue"),
  compartment = c(root = "darkorange4", shoot = "darkgreen"))

solA <- rep(c("no", "yes"), c(10,10))
compartment <- rep(c("root","shoot","root","shoot"), c(5,5,5,5))
sample <- names(DEG_counts)
sip <- data.frame(sample, solA, compartment)%>% column_to_rownames("sample")
...
clrs <- list(
  solA = c(n= "gray", y ="midnightblue"))

solA <- rep(c("n", "y"), c(5,5))
sample <- names(DEG_counts)
...
sip2 <- data.frame(names(DEG_counts2), solA) %>% column_to_rownames("names.DEG_counts2.")
