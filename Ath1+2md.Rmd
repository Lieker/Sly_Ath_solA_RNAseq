...
Let's combine the two datasets in one counts file.
```{r message = FALSE, warning = FALSE}
Ath1 <- read.csv(file = "Ath1/input/counts.csv",
                 header = TRUE,
                 stringsAsFactors = FALSE,
                 fileEncoding = "UTF-8-BOM")
Ath1 <- Ath1[,c(1:7,18:22)]
Ath2 <- read.csv(file = "Ath2/input/counts.csv",
                 header = TRUE,
                 stringsAsFactors = FALSE,
                 fileEncoding = "UTF-8-BOM")
Ath2 <- Ath2[,c(1:11)]
sum(Ath1$Geneid == Ath2$Geneid) #All Geneid's are the same, so it is safe to join these dataframe with left_join()
Ath12 <- left_join(Ath1, Ath2, by = "Geneid")
write_csv(Ath12, "Ath2/input/counts_Ath12.csv")
```

Let's start with PCA analysis.
```{r}
source("Ath2/scripts/create_pca_plot.R")
p1 <- plot_pca(count_csv_file = "Ath2/input/counts_Ath12.csv",
               xp_design_csv_file = "Ath2/input/xp_design_Ath12.csv",
               trm = c("solAno", "solAyes"),
               pca_colour = "treatment")
p2 <- plot_pca(count_csv_file = "Ath2/input/counts_Ath12.csv",
               xp_design_csv_file = "Ath2/input/xp_design_Ath12.csv",
               trm = c("solAno", "solAyes"),
...
Still, we can account for experiment number in the design formula. Let's see if we get any overlapping DEGs.

```{r}
source("Ath2/scripts/volcanoplot.R")
make_volcanoplot(counts_csv_file = "Ath2/input/counts_Ath12.csv",
               xp_design_csv_file = "Ath2/input/xp_design_Ath12.csv",
               trtm = c("solAno", "solAyes"),
               ref_treatment = "solAno",
               treatment2 = "solAyes",
               method = "Ath12",
               FCcutoff_volcano = 1,
               xsize = 5,
               ttl = "DEGs from Ath1 and Ath2ab combined",
               padj_threshold = 0.05)

```

There are a few significantly upregulated genes.
Let's see what they are.

```{r}
source("Ath2/scripts/get_annotated_DEGs.R")
w <- get_annotated_DEGs(counts_csv_file = "Ath2/input/counts_Ath12.csv",
               xp_design_csv_file = "Ath2/input/xp_design_Ath12.csv",
               trtm = c("solAno", "solAyes"),
               ref_treatment = "solAno",
               treatment2 = "solAyes",
