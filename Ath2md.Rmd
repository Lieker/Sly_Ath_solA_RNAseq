...
It seems that there are barely any DEGs that are caused by solA alone, if we take into account the interaction between solA and N. So, let's find out which DEGs come up if we only look at factor N:solA.
...
Now there are actually some DEGs.
...
Let's see if the other model we used for the Wald test are also verified by LRT: the model ~N+P+N:solA. How many DEGs are caused by the interaction between N and solA? (reduced model ~N+P)
```{r, warning = FALSE, message = FALSE}
compare_wald_vs_LRT(trtm = c("a","b","c","d","e","f","g"), method = "N:solA")
```

LRT gives many more DEGs and nearly all of the Wald test are in this list. So this model gives us a reliable set of DEGs.
...
## Finding groups of co-expressed (clustering) genes across samples

We use DEGpatterns package to find clustering genes across samples. We use the complete Ath2 dataset for this to find genes that express according to a certain pattern over the sample groups.
First, we have to extract the list of DEGs, filtered for Padj<0.05. Then we take our vst_transformed counts, only from the significantly DEGs.
...
clusters <- degPatterns(DEG_counts, metadata = xp_design, time = "solA")
clusters$plot
```
...
Let's also do the annotation and GO term annotation of the DEGs we found when using other models and subsets of samples.
First, the DEGs we find when using the N:solA term instead of solA term.
```{r message = FALSE, warning = FALSE}
x <- get_annotated_DEGs(trtm = c("a","b","c","d","e","f","g"), method = "N:solA")
GO_annotation_list(trtm = c("a","b","c","d","e","f","g"), method = "N:solA", plottype = "bar")
GO_annotation_list(trtm = c("a","b","c","d","e","f","g"), method = "N:solA", plottype = "go")
```

Then, do the same for all the comparisons within nutritional conditions:
```{r message = FALSE, warning = FALSE}
#for the non starvation samples
x <- get_annotated_DEGs(trtm = c("a","b"), method = "treatment")
GO_annotation_list(trtm = c("a","b"), method = "treatment", plottype = "bar")
GO_annotation_list(trtm = c("a","b"), method = "treatment", plottype = "go")

#for the only P starvation samples
x <- get_annotated_DEGs(trtm = c("c","d"), ref_treatment = "c", treatment2 = "d", method = "treatment")
GO_annotation_list(trtm = c("c","d"), ref_treatment = "c", treatment2 = "d", method = "treatment", plottype = "bar")
GO_annotation_list(trtm = c("c","d"), ref_treatment = "c", treatment2 = "d", method = "treatment", plottype = "go")

#for the N+P starvation samples
x <- get_annotated_DEGs(trtm = c("e","f"), ref_treatment = "e", treatment2 = "f", method = "treatment")
GO_annotation_list(trtm = c("e","f"), ref_treatment = "e", treatment2 = "f", method = "treatment", plottype = "bar")
GO_annotation_list(trtm = c("e","f"), ref_treatment = "e", treatment2 = "f", method = "treatment", plottype = "go")
