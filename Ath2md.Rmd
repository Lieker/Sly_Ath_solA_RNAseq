...
Figure 2
```{r}
p <- plot_pca(pca_colour = "N")
p + scale_shape_discrete(name = "treatment", labels = c("control", "solA")) +
  scale_color_manual(name = "N", labels = c("starvation", "replete"), values = c('#0099FF','#E69F00'))
```

Figure S2:
```{r}
p <- plot_pca(pca_colour = "P")
p + scale_shape_discrete(name = "treatment", labels = c("control", "solA")) +
  scale_color_manual(name = "P", labels = c("starvation", "replete"), values = c('#0099FF','#E69F00'))
```
...
p3 <- plot_pca(trm = c("e","f"), pca_colour = "solA", pc_x_axis = "PC1", pc_y_axis = "PC2")

p1 + scale_colour_manual(name = "treatment", labels = c("control", "solA"), values = c('#0099FF','#E69F00'))
p2 + scale_colour_manual(name = "treatment", labels = c("control", "solA"), values = c('#0099FF','#E69F00'))
p3 + scale_colour_manual(name = "treatment", labels = c("control", "solA"), values = c('#0099FF','#E69F00'))
```
...
```{r}
p1 <- plot_pca(pc_x_axis = "PC3", pc_y_axis = "PC4", pca_colour = "treatment", pca_shape = "solA")
...
P <- rep(c("replete","starvation","replete"), c(10,20,5)) 
N <- rep(c("replete","starvation"), c(20,15))
treatment <- rep(c("control","solA","control","solA","control","solA","control"), c(5,5,5,5,5,5,5))
samples <- names(DEG_counts)
sip <- data.frame(samples, treatment, N, P) %>% column_to_rownames("samples")

clrs <- list(
  P = c(starvation = "darkcyan", replete ="midnightblue"),
...
samples <- names(DEG_counts)
sip <- data.frame(samples, solA) %>% column_to_rownames("samples")
...
v <- venn.diagram(x,
             category.names = c(paste0("All (", nrow(all), ")"),
                                paste0("Replete (", nrow(replete), ")"),
                                paste0("P starv (", nrow(Pstarv), ")"), 
                                paste0("N+P starv (", nrow(NPstarv), ")")),
             filename = "Ath2/output/Ath2_venn.png",
             output = TRUE,
             col=c("#440154ff", '#21908dff', '#fde725ff', "#c52828ff"),
...
These are the GO terms enriched under replete nutritional conditions (Pval < 0.001).

```{r message = FALSE, warning = FALSE}
source("Ath2/scripts/summ_dotplot.R")
p <- sumplotdot(sign = 0.05, nutrcond = "all", updown = "down", show = "all")
p + ggtitle("GO terms enriched in downregulated DEGs")
```

Let's also look at the upregluated genes.

```{r message = FALSE, warning = FALSE}
source("Ath2/scripts/summ_dotplot.R")
p <- sumplotdot(sign = 0.05, nutrcond = "all", updown = "up", show = "all")
p + ggtitle("GO terms enriched in upregulated DEGs")
```

Summarize these results in one dotplot.

```{r message = FALSE, warning = FALSE}
source("Ath2/scripts/summ_dotplot.R")
library(patchwork)
p1 <- sumplotdot(sign = 0.05, nutrcond = "all", updown = "up", show = 10, double = "revigo")
p2 <- sumplotdot(sign = 0.05, nutrcond = "all", updown = "down", show = 10, double = "revigo")

p1 / p2
```

We can also summarize more than one nutritional condition in one plot. For example, let's look at all upregulated GO terms in all nutritional conditions:

```{r message = FALSE, warning = FALSE}
source("Ath2/scripts/summ_dotplot.R")
p <- sumplotdot(updown = "down", sign = 0.05, nutrcond = c("Pstarv","NPstarv","replete"), double = "revigo")

```

For the supplemental materials, a dot plot summarizing all enriched GO terms for the per nutritional condition comparisons is produced: 

```{r message = FALSE, warning = FALSE}
source("Ath2/scripts/summ_dotplot.R")
p <- sumplotdot(updown = c("down"), sign = 0.05, nutrcond = c("NPstarv","replete", "Pstarv"))
...
```{r message = FALSE, warning = FALSE}
source("Ath2/scripts/summ_dotplot.R")
p <- sumplotdot(updown = c("up"), sign = 0.05, nutrcond = "NPstarv")
