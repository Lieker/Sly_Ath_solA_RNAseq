...
```
There is an outlier in the group of solA on the right. This is sample S46. Let's check if it is really an outlier.
```{r outlier}
PCAdata <- read.csv("Ath2/input/dataFigureS2B.csv") %>% column_to_rownames("X")

(PCAdata[6,1] - mean(PCAdata[7:10,1])) / sd(PCAdata[7:10,1])

```
The outlier is more than 6x the sd distance from the other data points in the group, so should be removed.
...
```{r pca_nutr, fig.width = 10, fig.height = 12, message = FALSE, warning = FALSE}
...
p3 <- plot_pca(trm = c("a","g"), pca_colour = "treatment") + ggtitle("N starvation")
p4 <- plot_pca(trm = c("a","e","g"), pca_colour = "treatment") + ggtitle("N, N+P starvation and replete")
p5 <- plot_pca(trm = c("a", "c", "e", "g"), pca_colour = "treatment") + ggtitle("replete, P, N+P, N starvation")
p6 <- plot_pca(trm = c("a", "c", "e", "g"), pca_colour = "treatment", pc_x_axis = "PC3", pc_y_axis = "PC4") + ggtitle("replete, P, N+P, N starvation")
p7 <- plot_pca(trm = c("e","g"), pca_colour = "treatment") + ggtitle("N+P and N starvation")
p8 <- plot_pca(trm = c("c","e","g"), pca_colour = "treatment") + ggtitle("P, N+P and N starvation")

(p1 + scale_shape_discrete(guide = "none")) + (p2 + scale_shape_discrete(guide = "none")) +
(p3 + scale_shape_discrete(guide = "none")) + (p4 + scale_shape_discrete(guide = "none")) + p5 + p6 + p7 + p8 + plot_layout(ncol = 2, guides = "collect")
```

P5 is most interesting for the manuscript.

```{r}
p5 + guides(shape = guide_legend(override.aes = list(size = 3)),
            colour = guide_legend(override.aes = list(size = 3))) +
  scale_colour_manual(name = "treatment", 
                      labels = c("replete", "P starvation", "N+P starvation", "N starvation"), 
                      values =c("navyblue","goldenrod4","darkred","limegreen")) +
  scale_shape_manual(name = "N", labels = c("starvation", "replete"),values = c(16,18)) +
  ggtitle("PCA analysis starvation conditions")
...
```{r nutrstarv_DEGs, fig.width = 11, fig.height = 6, message = FALSE, warning = FALSE}
source("Ath2/scripts/volcanoplot.R")
v1 <- make_volcanoplot(trtm = c("a", "e"), ref_treatment = "a", treatment2 = "e", log2FC_threshold = 0, padj_threshold = 0.05, ttl = "N+P starvation")
v2 <- make_volcanoplot(trtm = c("a", "c"), ref_treatment = "a", treatment2 = "c", log2FC_threshold = 0, padj_threshold = 0.05, ttl = "P starvation")
v3 <- make_volcanoplot(trtm = c("a", "g"), ref_treatment = "a", treatment2 = "g", log2FC_threshold = 0, padj_threshold = 0.05, ttl = "N starvation")
v2 + v3 + v1 + plot_layout(guides = "collect")
...
dds_ag <- get_DESeq_dds(trtm = c("a", "g"), ref_treatment = "a", treatment2 = "g", method = "treatment")
res_ag <- get_unfiltered_res_dds(trtm = c("a", "g"), ref_treatment = "a", treatment2 = "g", method = "treatment")
...
Nstarv_nosolA <- get_annotated_DEGs(d = dds_ag, r = res_ag, name = "Ath2/output/annotated_DEGslist_ag.csv")
nrow(Pstarv_nosolA)
nrow(NPstarv_nosolA)
nrow(Nstarv_nosolA)
sum(Nstarv_nosolA$Group.1 %in% NPstarv_nosolA$Group.1)
```
...
```{r message = FALSE, warning = FALSE}
replete <- get_annotated_DEGs(d=dds_ab, r=res_ab)
Pstarv <- get_annotated_DEGs(d=dds_cd,r=res_cd)
NPstarv <- get_annotated_DEGs(d=dds_ef,r=res_ef)
```


Find all overlapping genes in the datasets
...
Check how many tomato and Arabidopsis genes overlap. First, get the Sly list of annotated genes in the Sly md file.

```{r overlap3, message = FALSE, warning = FALSE}
tomatoDEGs <- DEGSly$athaliana_eg_homolog_ensembl_gene
araDEGs <- rbind(replete, Pstarv, NPstarv)
overlapta <- araDEGs[araDEGs$Group.1 %in% tomatoDEGs, ]
write.csv(overlapta, "Ath2/output/overlap_ta.csv", row.names = FALSE)
```
...
Figure 3:
```{r, dotplot_NPstarvnosolA, fig.height = 5, fig.width = 9, message = FALSE, warning = FALSE}
source("Ath2/scripts/summ_dotplot.R")
p1 <- sumplotdot(updown = c("up"), sign = 0.05, nutrcond = c("Nstarv.nosola","NPstarv.nosola"), double = "revigo", show = 10)
p2 <- sumplotdot(updown = c("down"), sign = 0.05, nutrcond = c("Nstarv.nosola","NPstarv.nosola"), double = "revigo", show = 10)
...
First for the samples including solA

```{r message = FALSE, warning = FALSE}
all_Ath_genes <- unique(unlist(list(NPstarv$Group.1, Pstarv$Group.1, replete$Group.1, tomatoDEGs)))
all_Ath_genes <- str_subset(all_Ath_genes, "AT*")
repl <- all_Ath_genes %in% replete$Group.1
Pst <- all_Ath_genes %in% Pstarv$Group.1
NPst <- all_Ath_genes %in% NPstarv$Group.1
Sly_root <- all_Ath_genes %in% DEGroot$athaliana_eg_homolog_ensembl_gene
Sly_shoot <- all_Ath_genes %in% DEGshoot$athaliana_eg_homolog_ensembl_gene
b <- data.frame(all_Ath_genes, repl, Pst, NPst, Sly_root, Sly_shoot) 
b <- b %>% column_to_rownames(., var = "all_Ath_genes")

colnames(b) <- c("Ath_replete+solA","Ath_Pstarv+solA","Ath_NPstarv+solA","Sly_root+solA","Sly_shoot+solA")
...
Make upset plot for the samples only focused on nutritional starvation:

```{r, fig.height = 3, fig.width = 5, warning = FALSE, message = FALSE}
all_Ath_genes <- unique(unlist(list(Pstarv_nosolA$transcript, NPstarv_nosolA$transcript, Nstarv_nosolA$transcript)))
all_Ath_genes <- str_subset(all_Ath_genes, "AT*")
Pns <- all_Ath_genes %in% Pstarv_nosolA$transcript
NPns <- all_Ath_genes %in% NPstarv_nosolA$transcript
Nns <- all_Ath_genes %in% Nstarv_nosolA$transcript
d <- data.frame(all_Ath_genes, Pns, NPns, Nns) %>% column_to_rownames("all_Ath_genes")
colnames(d) <- c("Pstarv","NPstarv","Nstarv")
groups <- colnames(d)
up2 <- upset(d, 
             groups,
             set_sizes=(upset_set_size() + 
...
Find the genes that are present in all 3 treatments.

```{r}
N_P <- Pstarv_nosolA[Pstarv_nosolA$transcript %in% Nstarv_nosolA$transcript,]
N_P_NP <- NPstarv_nosolA[NPstarv_nosolA$transcript %in% N_P$transcript,]
write.csv(N_P_NP, "Ath2/output/nutrition_overlap.csv", row.names = FALSE)
```

Check which genes are present in both Pstarv_nosola vs. replete and in Pstarv vs Pstarv_nosola.

```{r}
Psol <- Pstarv[Pstarv$transcript %in% Pstarv_nosolA$transcript,]
...
Try to get plotcounts3

```{r, warning = FALSE, message = F}
source("Ath2/scripts/get_GOs.R")
source("Ath2/scripts/produce_GOdf.R")
source("Ath2/scripts/plot_pathwayplot.R")
x <- get_GOs(d = dds_all, rlist = list(res_ab,res_cd,res_ef))
x1 <- x[str_detect(x$name_1006, c("defense",
                                  "immun",
                                  "symbiont",
                                  "virus",
                                  "viral",
                                  ...
                                  "other organism")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in defense, immunity and symbiont relationships")
```

Do the same for genes involved in photosynthesis and carbon fixation.

```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("photosyn",
                                  "carbon fixation",
                                  "chloroplast",
                                  "light",
                                  "pentose-phosphate")),]
...
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in photosynthesis and carbon fixation")
```

Do the same for ethylene, auxin, gibberellin
```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("ethylene")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in ethylene biosynthesis and signalling")
```
```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("auxin")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in auxin biosynthesis and signalling")
```
```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("gibberellin")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in gibberellin biosynthesis and signalling")
```

```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("abscisic acid")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in abscisic\nacid signalling")
```

```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("hypoxia","low oxygen")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in response to hypoxia")
```

```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("cell wall")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in cell wall biogenesis")
```

```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("growth","development")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in growth and development")
```

```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("lateral root","root hair","root development")),]
GOs <- unique(x1$go_id)
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in root development")
```

```{r, message = FALSE, warning = FALSE}
x1 <- x[str_detect(x$name_1006, c("glutamine",
                                  "camalexin",
                                  "phenylpropanoid",
                                  "tryptophan",
                                  "glucosinolate",
...
t <- produce_GOdf(r = x, d = dds_all, GOid = GOs)
pp <- plot_pathwayplot(t)
pp + plot_annotation(title = "Genes involved in secondary metabolism")
```
